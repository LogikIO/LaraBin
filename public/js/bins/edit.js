...
function existingFiles() {
    $.each($('#editor-container .editor-instance'), function(){
        $editor = $(this);
        var uniqueId = new Date().getTime() + Math.floor(Math.random()*10000);
        var uniqueId = new Date().getTime() + Math.floor(Math.random()*10000);
        $editor.find('.editor-area').attr('id', 'editor_' + uniqueId);
        $editor.find('[name="file"]').attr('name', 'file['+uniqueId+']');
        $editor.find('[name="name"]').attr('name', 'name['+uniqueId+']');
        $editor.find('[name="language"]').attr('name', 'language['+uniqueId+']');
        $editor.find('[name="code"]').attr('name', 'code['+uniqueId+']');
        $editor.find('.language, .delete-file').data('editor', uniqueId);
        ...
        var textarea = $editor.find('.real-code');
        editor.getSession().setValue(textarea.val());
        editor.getSession().on('change', function(){
            textarea.val(editor.getSession().getValue());
        });
        var mode = $editor.find('.language').val();
